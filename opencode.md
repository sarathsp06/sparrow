...
Optional shared-secret auth via `SPARROW_API_KEY` env var. When set, every `/v1/*` REST request must include the key. When unset, all endpoints are open.

| Method | How to pass |
|--------|-------------|
| HTTP header | `X-API-Key: <key>` |

Excluded paths: `/health`, `/ready`, `/docs`, `/openapi`, UI catch-all. Uses constant-time comparison (`crypto/subtle`). The embedded UI gets the key injected at runtime via `window.__SPARROW_CONFIG__`.

```bash
curl -H "X-API-Key: my-secret" http://localhost:8080/v1/consumers/default/webhooks
```
...
Route-group middleware: API key auth only wraps the `/v1/*` group (`r.Group` in `cmd/server/main.go`), not health, docs, or the UI. See [route table](okf/packages/cmd-server.md) in OKF.
...
4. **REST error translation**: Use `mapError(ctx, err, msg)` from `internal/rest/errors.go`
5. **Tenant scoping**: Every query must filter by `tenant_id` (using `tenant.DefaultTenantID`)
6. **Consumer scoping**: Where applicable, queries also filter by consumer
...
### Naming
- **Files**: `snake_case.go` (e.g., `webhook.go` under `internal/rest/`)
- **Packages**: Lowercase single word (e.g., `consumer`, `tenant`)
- **REST OperationIDs**: `camelCase` verb-first (e.g., `registerWebhook`, `listDeliveries`)

## Known Gaps

1. No API-level rate limiting
2. No API versioning beyond URL path (`/v1/...`)
3. No payload size limits
4. No tenant usage quotas
5. No scheduled/delayed webhooks
6. Partial data retention (events purge via `SPARROW_EVENT_RETENTION_DAYS`; health events and batch jobs grow unbounded)
...
| UI Redesign | Feb 2026 | Terminal aesthetic, consumer chooser |
| Multi-Tenancy | Mar 2026 | Tenants, consumers, batch fan-out, pgxpool tuning, 6 composite indexes |
...
| REST Migration | Aug 2026 | Removed gRPC + Connect-RPC + proto, replaced with REST/OpenAPI (Huma), Scalar docs at `/docs`, deleted `docs/` Astro site |
