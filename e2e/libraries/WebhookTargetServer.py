"""
WebhookTargetServer -- Robot Framework library for programmable mock webhook endpoints.
...
class _Target:
    ...
    def _handle_request(self):
        ...
        # Capture delivery FIRST (even for error/slow responses, to count requests)
        ...
        # Handle slow behavior (sleep AFTER capturing)
    ...
    def _get_status_code(self, request_number: int) -> int:
        if self.behavior == "ok":
            ...
            return 200  # handled separately with delay
    ...
    def switch_behavior(self, new_behavior: str):
        """Switch behavior at runtime (e.g., fix a broken endpoint)."""
        self.behavior = new_behavior


class WebhookTargetServer:
    """Robot Framework library for managing mock webhook targets."""
    ...
    def start_webhook_target(self, name: str, behavior: str = "ok") -> str:
        """Start a mock webhook target. Returns the URL.

        Behaviors: ok, status_404, status_500, fail_then_succeed_N, slow_Xs, rate_limited_N
        """
        ...
        # Return host.docker.internal URL so Sparrow (in Docker) can reach the test machine
        return f"http://host.docker.internal:{port}/webhook"

    def wait_for_deliveries(self, name: str, count: int, timeout: str = "30s"):
        """Wait until target has received at least N deliveries."""
        timeout_sec = self._parse_timeout(timeout)
        target = self._targets[name]
        target.wait_for_deliveries(int(count), timeout_sec)

    def get_delivery_count(self, name: str) -> int:
        """Return number of deliveries received by target."""
        target = self._targets[name]
        with target._lock:
            return len(target.deliveries)

    def get_deliveries(self, name: str) -> list[dict]:
        """Return all captured deliveries as list of dicts."""
        target = self._targets[name]
        with target._lock:
            return [
                {
    ...
    def get_latest_delivery(self, name: str) -> dict:
        """Return the most recent delivery for a target."""
        target = self._targets[name]
        with target._lock:
    ...
    def switch_target_behavior(self, name: str, new_behavior: str):
        """Switch a target's behavior at runtime."""
        self._targets[name].switch_behavior(new_behavior)

    def stop_all_targets(self):
        """Stop all running targets. Call in suite teardown."""
        for target in self._targets.values():
            target.stop()
        self._targets.clear()

    def assert_no_deliveries(self, name: str, wait: str = "3s"):
        """Assert that a target received NO deliveries after waiting."""
        wait_sec = self._parse_timeout(wait)
        time.sleep(wait_sec)
        target = self._targets[name]
        with target._lock:
        ...
        if count > 0:
            raise AssertionError(
                f"Expected 0 deliveries for '{name}', but got {count}"
            )
