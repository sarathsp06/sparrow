<script lang="ts">
  ...
  const config: Record<WebhookHealth, { bg: string; text: string; dot: string; label: string; tooltip?: string }> = {
    [WebhookHealth.HEALTH_UNSPECIFIED]: { bg: 'bg-gray-100', text: 'text-gray-600', dot: 'bg-gray-400', label: '\u2014', tooltip: 'Health is calculated after the first deliveries' },
...
<span class="inline-flex items-center gap-1.5 rounded-full font-medium {config[health].bg} {config[health].text} {sizeClasses[size]}" title={config[health].tooltip ?? ''}>
