...
import { llmsBasePrompt, llmsRootDetails } from '../prompts.mjs';
...
export const GET: APIRoute = async (context) => {
  ...
  segments.push([
    ...
    ...starlightLllmsTxtContext.customSets.map(({ label, description, slug }) =>
...
- The content is automatically generated from the same source as the official documentation`);
  if (starlightLllmsTxtContext.optionalLinks.length > 0) {
    ...
    segments.push(starlightLllmsTxtContext.optionalLinks.map((link) => `- [${link.label}](${link.url})${link.description ? `: ${link.description}` : ''}`).join('\n'));
  }
