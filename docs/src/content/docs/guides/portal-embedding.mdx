...
<ThemeDiagram
  ...
  alt="Sequence diagram tracing one request: the browser sends a bearer-authenticated GET to the proxy; the proxy matches the /v1/consumers/ allowlist and forwards to Sparrow over the VPN; Sparrow verifies the token signature and expiry and that the path belongs to the consumer, then returns 200 and JSON; the proxy streams the response back to the browser unchanged."
/>
