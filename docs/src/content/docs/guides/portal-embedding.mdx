...
2. **Your backend mints a token** over the private network:
   ...
   # → { "token": "spt_v2....", "expires_at": "...", "path": "/portal#token=spt_v2...." }
   ```
...
5. **The portal calls the API through the same proxy**, under the single
   ...
   request. Sparrow's portal gateway verifies the HMAC signature, key ID, and expiry
...
- **Token integrity** — tokens are HMAC-SHA256 signed with
  the primary configured encryption key from `SPARROW_ENCRYPTION_KEYS` and verified in constant time. Tampering with the
