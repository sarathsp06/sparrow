...
import eventPipelineLight from '../../../assets/diagrams/event-pipeline-light.svg';
import eventPipelineDark from '../../../assets/diagrams/event-pipeline-dark.svg';
...
<ThemeDiagram
  light={eventPipelineLight}
  dark={eventPipelineDark}
  alt="Flow: Push Event goes to the Event Worker, which finds subscriptions and creates deliveries; the Webhook Worker then either records the result on success or retries with backoff on failure."
/>
...
Event registrations act as a schema registry. Pushing an unregistered event type auto-registers it; pushing an event type that was explicitly deactivated is rejected.
