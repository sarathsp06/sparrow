...
Sparrow has four core concepts that work together to deliver webhooks reliably.
...
- **At-least-once delivery** — retryable failures (server errors, timeouts, connection issues) are automatically retried. Non-retryable failures (DNS errors, TLS errors, 4xx responses) are recorded and not retried.
