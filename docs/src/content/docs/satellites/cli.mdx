...
| `push` | `POST /v1/namespaces/{ns}/events?event=<name>`; on `404 unknown event type` → `POST /v1/event-types`, then retries the push once |
| `tail deliveries` | `GET /v1/namespaces/{ns}/deliveries` every 2 s, printing rows not seen before; webhook IDs are resolved to URLs via `GET /v1/namespaces/{ns}/webhooks` |
| `listen` | `POST /v1/namespaces/{ns}/webhooks` (temporary registration) … `DELETE /v1/namespaces/{ns}/webhooks/{id}` on Ctrl-C |
| `use` | `POST /v1/namespaces/{ns}/webhooks`, then `GET` + `PATCH /v1/namespaces/{ns}/subscriptions/{id}` to attach the recipe's transform template and label filters |
...
subscription(s). Recipes are looked up as `--file`, then `./satellites/recipes/<name>.yaml`,
