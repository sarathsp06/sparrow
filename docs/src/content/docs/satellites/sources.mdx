...
ingest server (:8787):
  POST /ingest/stripe                    POST /ingest/github
...
ingest:
  listen: :8787                # HTTP listen address for webhook ingest
  providers:
    stripe:
...
1. In the Stripe dashboard, go to **Developers → Webhooks → Add endpoint** and set the URL to `https://<your-sources-host>/ingest/stripe`.
2. Select the event types you want (or all).
3. After creating the endpoint, reveal its **Signing secret** (`whsec_...`) and put it in `ingest.providers.stripe.signing_secret`.
...
2. Payload URL: `https://<your-sources-host>/ingest/github`. Content type: `application/json`.
3. Set a **Secret** and put the same value in `ingest.providers.github.secret`.
