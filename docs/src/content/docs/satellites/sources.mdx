---
title: Sources
description: Turn cron schedules, Stripe webhooks, GitHub webhooks, and Kafka messages into Sparrow events with the sparrow-sources binary.
---
...
- **Kafka** — consume messages from Kafka topics and re-publish them as Sparrow events.
...
kafka:
  - brokers: [localhost:9092]   # seed brokers (default: ["localhost:9092"])
    topic: events               # Kafka topic to consume (default: "events")
    group_id: sparrow-sources   # consumer group ID (default: "sparrow-sources")
    sparrow_event_prefix: kafka # prefix of the OUTBOUND Sparrow event name (default: "kafka")
    labels: {env: production}   # optional custom labels attached to pushed events
```
...
| Provider / Source | Incoming | Sparrow event |
|-------------------|----------|---------------|
...
| Kafka | topic `orders` | `kafka.orders` |
...
Add a Kafka consumer source under `kafka:` in `sources.yaml`:
...
kafka:
  - brokers:
      - localhost:9092
    topic: events
    group_id: sparrow-sources
    sparrow_event_prefix: kafka
```

The consumer fetches messages from Kafka, parses JSON payloads (or wraps raw message strings in `{"value": ...}`), attaches labels (`source=kafka`, `topic`, `key`), and pushes them into Sparrow. Offsets are committed after successful push into Sparrow.
