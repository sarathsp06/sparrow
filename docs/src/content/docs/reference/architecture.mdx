...
The KEK is resolved on startup:

1. `SPARROW_ENCRYPTION_KEY` environment variable (64-char hex = 32 bytes)
2. If not set, a temporary key is auto-generated for the session and printed to stdout

The key is **never** stored in the database. Storing the encryption key next to the data it protects defeats the purpose of encryption at rest. If no env var is set, the generated key is ephemeral — restarting without setting the env var produces a new key and any previously encrypted data becomes unreadable.
