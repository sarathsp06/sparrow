...
`tenant`, `namespace`, and `webhooks` never import each other -- zero dependency cycles.
...
**`internal/grpc`** -- gRPC service implementations. The only package that imports all three domain packages.
...
`cmd/server/main.go` is the only file that imports all three domain packages. It constructs repositories, services, and wires them together with explicit constructor functions -- no framework.
