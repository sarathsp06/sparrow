# Graph Report - organic-lamprey  (2026-09-20)

## Corpus Check
- 360 files · ~315,429 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2621 nodes · 5324 edges · 196 communities (160 shown, 36 thin omitted)
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 766 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `ec8d8186`
...
1. `Errorf()` - 159 edges
2. `RepositoryInterfaceWithTracing` - 75 edges
...
Cohesion: 0.11
Nodes (10): Context, Span, Time, UUID, WebhookRegistration, NewRepositoryInterfaceWithTracing(), RepositoryInterfaceWithTracing, WebhookDelivery (+2 more)
...
Cohesion: 0.11
Nodes (13): toWebhookOutFromDomain(), DerefBoolOr(), DerefIntOr(), Duration, Time, Value, WebhookRegistration, IntArray (+5 more)
...
Cohesion: 0.09
Nodes (17): Int64Array, Time, UUID, Value, BatchJobData, BatchJobStatus, ConsumerStats, DeliveryFilter (+9 more)
...
Cohesion: 0.05
Nodes (42): API Changes, Commands, Completed Parts (v0.8.0 -- v1.2.1), Configuration, Configuration, Current State (as of v1.2.1), Decisions Log, Design (+34 more)
...
Cohesion: 0.06
Nodes (34): @astrojs/sitemap, @astrojs/starlight, @astrojs/svelte, dependencies, astro, @astrojs/sitemap, @astrojs/starlight, @astrojs/svelte (+26 more)
...
### Community 51 - "RunAllMigrations"
Cohesion: 0.46
Nodes (6): main(), Context, Logger, RunAllMigrations(), RunAppMigrations(), RunRiverMigrations()

### Community 52 - "kvFlag"
Cohesion: 0.15
Nodes (6): parseJSONArg(), T, TestKVFlag(), TestParseJSONArg(), kvFlag, listFlag
...
Cohesion: 0.16
Nodes (20): Int64Counter, Int64UpDownCounter, DefaultConfig(), GetMeter(), GetTracer(), Context, Tracer, newLoggerProvider() (+12 more)
...
Cohesion: 0.24
Nodes (10): T, WebhookTemplateContext, sampleContext(), substituteParams(), TestRecipes(), TestSendGridActivationParams(), tokenRefs(), T (+2 more)
...
Cohesion: 0.12
Nodes (9): consumerStore, current, known, beat, data, pulseStore, Telemetry, Recipe (+1 more)
...
### Community 147 - "parseUUID"
Cohesion: 0.16
Nodes (9): Context, WebhookService, Context, WebhookService, UUID, Context, WebhookService, parseUUID() (+1 more)

### Community 148 - "go-template-engine.ts"
Cohesion: 0.14
Nodes (17): applyFilterStage(), evaluateExpression(), evaluateToken(), EventContext, extractIfElseBlocks(), extractMatchingEnd(), goPrintf(), parseTokens() (+9 more)
...
## Knowledge Gaps
- **402 isolated node(s):** `DEFAULT_TOLERANCE_SECONDS`, `SignatureVerificationError`, `Headers`, `name`, `type` (+397 more)
...
- **Why does `Errorf()` connect `Errorf` to `setupEnv`, `testContext`, `Config`, `PrepareDeliveryRequest`, `webhooks/models.go`, `NewService`, `Context`, `parseUUID`, `NewWebhookHandler`, `parseDateFilter`, `ValidateIP`, `BatchJob`, `apiClient`, `EventProcessingWorker`, `ClassifyError`, `envelope`, `VerifyHMAC`, `Context`, `WebhookWorker`, `EventRecord`, `Context`, `WebhookService`, `TemplateEngine`, `runPush`, `RunAllMigrations`, `kvFlag`, `newEmailSink`, `otel.go`, `EventSubscription`, `newOTLPSink`, `newPalette`, `Client`, `BatchJobWorker`, `runInit`, `ServiceError`, `config`, `newS3Sink`, `.CreateWebhook`, `runUse`, `runListen`, `GetFunctionMap`, `newTailCmd`, `runFunctions`, `NewManager`, `WebhookService`, `jobInserter`, `runTemplateTest`?**
...
- **Why does `NewManager()` connect `NewManager` to `setupEnv`, `Errorf`, `Client`, `BatchJobWorker`, `EventProcessingWorker`, `NewService`, `RepositoryInterface`, `RetentionWorker`?**
  _High betweenness centrality (0.051) - this node is a cross-community bridge._
- **Are the 156 inferred relationships involving `Errorf()` (e.g. with `.EncryptionKeyring()` and `.Validate()`) actually correct?**
  _`Errorf()` has 156 INFERRED edges - model-reasoned connections that need verification._
